...
def cornersHeuristic(state, problem):
    ...
    unvisited_corners = set(corners).difference(visited_corners)

    if unvisited_corners:
        return max([abs(x1-x)+abs(y1-y) for x1, y1 in unvisited_corners])
    else:
        return 0
...
class AnyFoodSearchProblem(PositionSearchProblem):
    ...
    def isGoalState(self, state):
        ...
        """
        x, y = state
        return (x, y) in set(self.food.asList())
