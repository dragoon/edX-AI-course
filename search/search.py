...
def uniformCostSearch(problem):
    ...
    while not fringe.isEmpty():
        ...
        if problem.isGoalState(node):
            return actions
        if node not in closed:
            for child, action, cost1 in problem.getSuccessors(node):
                fringe.push((child, actions + [action], cost+cost1), cost+cost1)
            closed.add(node)
    util.raiseNotDefined()
...
def aStarSearch(problem, heuristic=nullHeuristic):
    ...
    while not fringe.isEmpty():
        ...
        if node not in closed:
            for child, action, cost1 in problem.getSuccessors(node):
                fringe.push((child, actions + [action], cost+cost1), cost+cost1+heuristic(child, problem))
            closed.add(node)
    util.raiseNotDefined()
